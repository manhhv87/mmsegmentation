...
class GLTB(nn.Module):
    def __init__(self, dim=256, mlp_ratio=4., drop=0.,
                 drop_path=0., act_layer=nn.ReLU6, 
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp_decoder(in_features=dim, hidden_features=mlp_hidden_dim,
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # self.b3 = Block(dim=decode_channels, num_heads=16,
        #                 window_size=window_size)
        # self.p3 = WF(encoder_channels[-2], decode_channels)

        # self.b2 = Block(dim=decode_channels, num_heads=16,
        #                 window_size=window_size)
        # self.p2 = WF(encoder_channels[-3], decode_channels)

        # self.p1 = FeatureRefinementHead(encoder_channels[-4], decode_channels)

        # self.segmentation_head = nn.Sequential(ConvBNReLU(
...
class FTFloodNet(BaseModule):
    def __init__(self,
        ...
        self.backbone = SwinTransformer(embed_dim=embed_dim, depths=depths, num_heads=num_heads, 
                                        window_size=window_size, frozen_stages=freeze_stages)
