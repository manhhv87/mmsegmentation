...
class Attention(nn.Module):
    def __init__(self, dim, head=8, axial=False):
        super(Attention, self).__init__()
        self.axial = axial
        self.dim = dim
        self.num_head = head
        self.attention_head_size = int(self.dim / head)
...
class WinAttention(nn.Module):
    def __init__(self, dim, head=8, win_size=4):
        super(WinAttention, self).__init__()
        self.window_size = win_size
        self.attention = Attention(dim, head)
...
class CSAttention(nn.Module):
    def __init__(self, dim, head=8, win_size=4):
        ...
        self.win_atten = WinAttention(dim, head, win_size)
        self.dlightconv = DlightConv(dim, win_size)
        self.global_atten = Attention(dim, head, axial=True)
...
class EAmodule(nn.Module):
    def __init__(self, dim, head=8, win_size=4):
        ...
        self.CSAttention = CSAttention(dim, head, win_size)
        self.EAttention = MEAttention(dim, head)
...
class MTUNet(BaseModule):
    def __init__(self,
        ...
        self.bottleneck = nn.Sequential(EAmodule(bottleneck, num_heads, win_size),
                                        EAmodule(bottleneck, num_heads, win_size))
