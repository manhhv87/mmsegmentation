...
class UpSample(nn.Module):
    def __init__(self, input_resolution, in_channels, scale_factor):
        ...
        if self.factor == 2:
            self.conv = nn.Conv2d(
                in_channels, in_channels//2, 1, 1, 0, bias=False)
            ...
            self.conv = nn.Conv2d(
                2*in_channels, in_channels, 1, 1, 0, bias=False)
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            self.upsample = UpSample(
                input_resolution, in_channels=dim, scale_factor=2)
        else:
            self.upsample = None
