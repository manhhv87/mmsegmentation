...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.bnk4 = Bottleneck(encoder_channels[-1], decode_channels)        
        self.glbt3 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)    # GLTB, number of heads h are both set to 8
        
        self.bnk3 = Bottleneck(encoder_channels[-2], decode_channels)
        self.ws3 = WS(decode_channels, decode_channels)     # weight sum        
        self.glbt2 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)

        self.bnk2 = Bottleneck(encoder_channels[-3], decode_channels)
        self.ws2 = WS(decode_channels, decode_channels)
        self.glbt1 = GLTB(dim=decode_channels, num_heads=8, window_size=window_size)

        self.bnk1 = Bottleneck(encoder_channels[-4], decode_channels)
        self.ws1 = WS(decode_channels, decode_channels)
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        x = self.glbt3(self.bnk4(res4))
        x = self.ws3(x, self.bnk3(res3))
        
        x = self.glbt2(x)
        x = self.ws2(x, self.bnk2(res2))

        x = self.glbt1(x)
        x = self.ws1(x, self.bnk1(res1))
