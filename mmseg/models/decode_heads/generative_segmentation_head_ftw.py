...
from mmseg.models.utils.dalle_d_vae import unmap_pixels
...
class GenerativeSegHeadFTW(GenerativeSegHeadFF):
    ...
    def __init__(self,
        ...
        _, self.post_swin_ln = build_norm_layer(
            dict(type='LN'), self.post_seg_channel)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        with torch.no_grad():
            ...
            vq_logits = self.forward(
                x).view(-1, self.vocab_size, h, w).contiguous()
            # get the pixel-wise prediction from indice prediction
            pixel_segmap_from_indice_pred = self.d_vae.decode(
                vq_logits.argmax(1).unsqueeze(1), img_size=[h, w]).contiguous()
            pixel_segmap_from_indice_pred = unmap_pixels(
                torch.sigmoid(pixel_segmap_from_indice_pred[:, :3]))
