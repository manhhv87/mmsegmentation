...
class GenerativeSegHeadFF(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(GenerativeSegHeadFF, self).__init__(init_cfg=init_cfg,
                                                  input_transform='multiple_select', channels=channels, **kwargs)
        ...
        self.conv_before_seg.conv.weight = nn.Parameter(
            self.conv_before_seg.conv.weight.contiguous())

        self.conv_seg = nn.Conv2d(
            self.indice_cls_channel, self.vocab_size, kernel_size=1)

        self.conv_seg_pixel = nn.Conv2d(
            channels, self.num_classes, kernel_size=1)

        _, self.swin_ln = build_norm_layer(
            dict(type='LN'), self.indice_seg_channel)
        ...
        for idx in range(len(inputs)):
            ...
            outs_for_pixel.append(
                resize(
                    ...
                    mode=self.interpolate_mode,
                    align_corners=self.align_corners))
        out_for_pixel = self.fusion_conv_pixel(
            torch.cat(outs_for_pixel, dim=1))
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        pixel_logits = self.conv_seg_pixel(
            x_p).view(-1, self.num_classes, h_p, w_p)
        pixel_logits = F.interpolate(
            pixel_logits, size=(h * 8, w * 8), mode='bilinear')

        # get vq indices from gt by dalle
        with torch.no_grad():
            # interpolate only preform when 1/4 scale was used
            gt_semantic_seg[gt_semantic_seg ==
                            self.pixel_ignore_index] = self.num_classes
            ...
            gt_semantic_seg[gt_semantic_seg ==
                            self.num_classes] = self.pixel_ignore_index
            ...
            gt_semantic_seg_for_recon[gt_semantic_seg !=
                                      self.pixel_ignore_index] = gt_semantic_seg[gt_semantic_seg != self.pixel_ignore_index].clone()
            gt_semantic_seg_for_recon[gt_semantic_seg ==
                                      self.pixel_ignore_index] = pixel_pred[gt_semantic_seg == self.pixel_ignore_index].clone()
            gt_semantic_seg_indices = self.get_gt_vq_indices(
                gt_semantic_seg_for_recon).unsqueeze(1)  # % 100

            # get ignore mask
            ignore_map = torch.ones_like(
                gt_semantic_seg, device=gt_semantic_seg.device)
            ignore_map[gt_semantic_seg >= self.num_classes] = 0
            ignore_mask = F.max_pool2d(
                ignore_map.float(), kernel_size=(8, 8), stride=(8, 8))
            indice_map_mask = ignore_mask

            # get final gt indices
            masked_gt_semantic_seg_indices = gt_semantic_seg_indices.clone()
            masked_gt_semantic_seg_indices[indice_map_mask ==
                                           0] = self.indice_ignore_index

            gt_semantic_seg_indices[ignore_mask ==
                                    0] = self.indice_ignore_index
    ...
    def forward_test(self, inputs, img_metas, gt_semantic_seg, test_cfg):
        ...
        seg_pred = self.decode_from_segmap(torch.tensor(
            rec_segmap), keep_ignore_index=False, prob=False)
    ...
    def _forward_test_recon_with_dalle(self, gt_semantic_seg, img_metas):
        ...
        for gt_semantic_seg_item in gt_semantic_seg:
            input_segmap = map_pixels(
                self.encode_to_segmap(gt_semantic_seg_item) / 255.0)
            ...
            rec_segmap = unmap_pixels(torch.sigmoid(rec_segmap[:, :3])) * 255
            seg_indices = self.decode_from_segmap(
                rec_segmap, keep_ignore_index=False)
            seg_logist = F.one_hot(seg_indices.to(torch.int64), self.num_classes).squeeze(1).permute(0, 3, 1, 2).to(
                torch.float)
            seg_logist = F.interpolate(
                seg_logist, size=gt_semantic_seg_item.shape[-2:], mode='bilinear')
    ...
    def decode_from_segmap(self, segmap, keep_ignore_index, prob=False):
        ...
        if keep_ignore_index:
            segmap = torch.Tensor.repeat(
                segmap, self.num_classes + 1, 1, 1, 1, 1).permute(1, 0, 2, 3, 4)
        else:
    ...
    def losses(self,
        ...
        if self.sampler is not None:
            indice_seg_weight = self.sampler.sample(
                indice_seg_logit, masked_indice_seg_label)
            dense_indice_seg_weight = self.sampler.sample(
                indice_seg_logit, full_indice_seg_label)
            pixel_seg_weight = self.sampler.sample(
                pixel_seg_logit, pixel_seg_label)
        else:
