...
class SMPUnet(BaseSegmentor):
    ...
    def whole_inference(self, img, img_meta, rescale):
        ...
        if rescale:
            ...
            if 'pad_shape' in img_meta[0] and img_meta[0]["pad_shape"] != img_meta[0]["img_shape"]:
                seg_logit = seg_logit[..., :img_meta[0]
                                      ["img_shape"][0], :img_meta[0]["img_shape"][1]]
            seg_logit = resize(
                seg_logit,
                size=size,
                mode='bilinear',
    ...
    def aug_test(self, imgs, img_metas, rescale=True, **kwargs):
        ...
        seg_logit, losses = self.inference(
            imgs[0], img_metas[0], rescale, **kwargs)
        for i in range(1, len(imgs)):
            cur_seg_logit, cur_loss = self.inference(
                imgs[i], img_metas[i], rescale)
...
class SMPUnetPlusPlus(SMPUnet):
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SMPUnetPlusPlus, self).__init__(backbone, decode_head, neck,
                                              auxiliary_head, train_cfg, test_cfg, ignore_index, pretrained, init_cfg)
        encoder_name = backbone.get("type")
        encoder_depth = backbone.get("depth", 5)
        decoder_channels = decode_head.get("channels", (256, 128, 64, 32, 16))
        decoder_use_batchnorm = decode_head.get("use_batchnorm", True)
